...
WCN 2.0 introduces three major architectural changes: regionalized quorums, blockchain-based consensus, and separation of coordinator and database roles. This section describes each component in detail.

\subsection{Regionalized Quorum Model}

In WCN 1.0, all writes were replicated globally across all nodes, ensuring strong consistency but introducing unnecessary latency for region-specific workloads. WCN 2.0 implements a flexible quorum system where writes can be committed to either a regional quorum or a global quorum depending on the access pattern.

For a system with $R$ regions and $N$ total nodes, let $N_r$ denote the number of nodes in region $r$. A regional quorum for region $r$ requires:

\begin{equation}
    Q_r = \left\lfloor \frac{N_r}{2} \right\rfloor + 1
\end{equation}

nodes within that region, while a global quorum requires:

\begin{equation}
    Q_g = \left\lfloor \frac{N}{2} \right\rfloor + 1
\end{equation}

nodes across all regions.

The system automatically promotes regional writes to global writes when cross-region access is detected, maintaining consistency while optimizing for common-case latency. This promotion mechanism is described in Algorithm~\ref{alg:quorum}.

\begin{algorithm}
\caption{Regionalized Quorum Write Protocol}
\label{alg:quorum}
\begin{algorithmic}[1]
\Require Key $k$, value $v$, client region $r_c$
\Ensure Value written with appropriate quorum
\State $r_k \gets$ GetKeyRegion($k$)
\If{$r_k = \textsc{null}$ \textbf{or} $r_k = r_c$}
    \State WriteRegional($k$, $v$, $r_c$, $Q_{r_c}$)
    \State SetKeyRegion($k$, $r_c$)
\Else
    \State PromoteToGlobal($k$)
    \State WriteGlobal($k$, $v$, $Q_g$)
\EndIf
\end{algorithmic}
\end{algorithm}
...
\subsection{Coordinator-Database Separation}

WCN 2.0 disaggregates functionality into two distinct components:

\textbf{WCN Node (Coordinator):} Maintains the blockchain ledger, participates in consensus, and coordinates state transitions. Coordinators are stateless except for the blockchain itself, making them lightweight and horizontally scalable.

\textbf{WCN Database (Storage):} Stores the actual key-value data and serves read queries. Databases subscribe to the blockchain to maintain eventually consistent state and can scale independently of coordinators.

This separation provides several operational benefits:
\begin{itemize}
    \item \textbf{Independent upgrades}: Database and coordinator deployments are decoupled, reducing deployment risk
    \item \textbf{Elastic storage scaling}: Additional database replicas can be added without affecting consensus
    \item \textbf{Fault isolation}: Database failures do not impact consensus, and vice versa
\end{itemize}
